...
const listkey = ["p3mu14", "arif", "arifgans"];
...
router.get('/cecan', async (req, res, next) => {
          var apikey = req.query.apikey
       	if(!apikey) return res.sendFile(__path + '/docs/403.html')
        if(listkey.includes(apikey)){
       fetch(encodeURI(`https://raw.githubusercontent.com/p3mu14-ui/v3/main/lib/cecan.js`))
        .then(response => response.json())
        .then(async data => {
...
router.get('/asupan', async (req, res, next) => {
          var apikey = req.query.apikey
       	if(!apikey) return res.sendFile(__path + '/docs/403.html')
        if(listkey.includes(apikey)){
       fetch(encodeURI(`https://raw.githubusercontent.com/p3mu14-ui/v3/main/lib/asupan.js`))
        .then(response => response.json())
        .then(async data => {
        var result = data[Math.floor(Math.random() * data.length)];
        var buffer = result.url;
